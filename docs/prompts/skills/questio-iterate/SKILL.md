...
description: >
  Execute-and-evaluate cycle within a human feedback loop. Modify parameters
  or code, run the pipeline or notebook, assess output against grounded
  expectations, report to human, and wait for direction before the next cycle.
metadata:
  short-description: iterative analysis with human feedback at each step
  tags: [questio, iterate, research, pipeline]
  tooling:
    mcp:
      - server: projio
        tools:
