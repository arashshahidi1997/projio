...
description: >
  Agent-driven deep dive into an anomaly, failure, or unexpected output.
  Guides systematic investigation: scope the issue, gather context, inspect
  outputs, compare against expectations, narrow the cause, and report findings.
metadata:
  short-description: investigate anomalies and failures in pipeline outputs
  tags: [questio, investigate, research, debugging]
  tooling:
    mcp:
      - server: projio
        tools:
