...
# Convention footprint (per-project)
plan/
...
# Auto-generated docs (output artifacts, not hand-edited)
docs/plan/
  questions.md                # rendered from questions.yml
  milestones.md               # rendered from milestones.yml
  roadmap.md                  # mermaid diagram from dependency graph
  evidence.md                 # evidence index grouped by question
...
1. Convert existing markdown tables → `plan/questions.yml` + `plan/milestones.yml`.
...
- Worklog **may optionally read** `plan/questions.yml` and `plan/milestones.yml` to derive goal progress. This is a one-way data flow: project → worklog, never the reverse.
...
- **Deliverable:** pixecog has `plan/questions.yml`, `plan/milestones.yml`, and a few `docs/log/result/` notes.
...
- `agent_instructions()` detects `plan/questions.yml` and includes a questio summary in session context.
