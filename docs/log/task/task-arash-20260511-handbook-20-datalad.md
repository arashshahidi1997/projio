---
title: "Handbook draft: 20-datalad — super/subdatasets, siblings, code-as-subdataset"
date: 2026-05-11
timestamp: 20260511-handbook-20-datalad
status: done
result_note: /storage2/arash/worklog/workflow/captures/20260511-164601-b76a39/note.md
completed: 2026-05-11T16:46:07+02:00
actionable: true
prompt: ""
...
blocked: false
blocked_by: ""
tags: [handbook, writing, stack-axis, agentic-workshop-2026-09, chapter-20-datalad]
model_hint: sonnet
---

## Task

Draft prose for every sub-chapter under `docs/handbook/20-datalad/`. Replace
...
- `teaching/NeuroPySeminar/docs/slides/bootcamp-ii/tutorial-plan.md` steps 1, 4 — `datalad install`, derivative subdataset creation, `datalad run`, `datalad rerun`. Reusable outline; refresh CLI examples for current datalad version.
...
| File | Words | Beats |
|---|---|---|
| `superdataset-and-subdatasets.md` | 600–900 | git + git-annex split; `.gitmodules` walkthrough on `gecog/.gitmodules`; super/sub composition (every flow is a subdataset); what changes vs plain git (annex stores large blobs, content fetched on demand); when to make a new subdataset |
| `siblings-and-ria.md` | 500–800 | RIA stores (file:// and ssh://); GitHub/GitLab siblings; preview-first sibling helpers from projio (`mcp__projio__datalad_siblings`); the asymmetry of push (data + metadata) vs publish (metadata only); permissions sync |
| `code-as-subdataset.md` | 400–700 | `code/lib/<name>/` as a mounted subdataset (from `pixecog/.gitmodules` `cogpy/labbox/labpy` rows); pinned commits; reproducing an analysis = checking out a specific commit of code + data; deliberate choice vs `pip install` |
