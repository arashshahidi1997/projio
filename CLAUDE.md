...
- `mcp/biblio.py` — `citekey_resolve`, `paper_context`, `paper_absent_refs`, `library_get`, `biblio_ingest`, `biblio_library_set`, `biblio_merge`, `biblio_compile`, `biblio_pdf_fetch`, `biblio_pdf_fetch_oa`, `biblio_pdf_fetch_oa_status`, `biblio_pdf_validate`, `biblio_docling`, `biblio_docling_batch`, `biblio_docling_status`, `biblio_grobid`, `biblio_grobid_check`, `biblio_graph_expand`, `biblio_rag_sync`, `biblio_library_quality`, `biblio_discover_authors`, `biblio_discover_institutions`, `biblio_institution_works`, `biblio_institution_authors`, `biblio_author_papers`, `biblio_pool_promote`, `biblio_zotero_pull`, `biblio_zotero_push`, `biblio_zotero_status`, `biblio_enrich`, `biblio_enrich_topic_tags`
...
- `mcp/pipeio.py` — ~60 tools across flow/mod/rule/config/notebook/docs/execution/paths. Notebook tools support multiple backends (jupytext percent-format and marimo) via a `NotebookBackend` protocol — format is per-notebook in `notebook.yml`. Key additions: `pipeio_target_paths`, `pipeio_dag_export` (SVG auto-writes to docs/), `pipeio_flow_audit` (read-only spec compliance check against [pipeline-docs.md](packages/pipeio/docs/specs/pipeio/pipeline-docs.md)), `pipeio_mod_audit`, `pipeio_mod_doc_refresh`, `pipeio_script_create`, `pipeio_nb_promote`, `pipeio_nb_move`, `pipeio_nb_extract` (extract figures from executed notebook — renamed from `pipeio_nb_report`), `pipeio_nb_validate` (structural validation for both formats), `pipeio_nb_watch` (marimo live editing), `pipeio_nb_snapshot` (execute marimo notebook and read cell outputs — agent's "eyes"), `pipeio_flow_report` (snakemake HTML report with size pre-flight — renamed from `pipeio_report`). No `pipe` parameter — flows addressed by name only. See `skill_read("pipeio-guide")` for full reference.
